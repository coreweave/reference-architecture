...
Deploy CoreWeave infrastructure as code: **VPC**, **CKS cluster**, optional **Object Storage bucket**, **NodePool**, and **DFS (Distributed File Storage)** PVC. Single Terraform root with modules; two-phase apply (cluster first, then NodePool/DFS with kubeconfig).
...
| **NodePool** | No | CKS node pool (via Kubernetes manifest). Created in **phase 2** after the cluster exists and you have kubeconfig. |
...
2. In `terraform.tfvars`:
   - Set **cks_kubeconfig_path** to the path of the downloaded file (e.g. `"~/.kube/config"` or `"/path/to/cks-kubeconfig.yaml"`).
   - Set **create_nodepool = true** and/or **create_dfs_pvc = true** if you want them.
3. Apply again:
...
Terraform will create the NodePool and/or DFS PVC(s) in the cluster. If you see errors like `no such host`, the kubeconfig is still pointing at an old or wrong cluster; download a fresh kubeconfig for the current cluster.
...
| 8 | Run `terraform apply` again to create NodePool and/or DFS PVC. |
...
terraform state mv 'kubernetes_manifest.nodepool_example[0]' 'module.nodepool.kubernetes_manifest.nodepool[0]' 2>/dev/null || true
...
- **nodepool_name** – If NodePool was created (from `module.nodepool`)
