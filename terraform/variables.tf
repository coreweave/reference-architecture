...
variable "create_nodepool" {
  type        = bool
  description = "Create the NodePool via Kubernetes provider. Set to false when cluster is missing or kubeconfig points at a deleted cluster (e.g. before first apply or after recreating cluster)."
  default     = true
}
