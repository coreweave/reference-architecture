...
def _(os, shell, time):
    def run_download_test():
        ...
        start = time.time()
        shell("curl -o /tmp/bandwidth-test/10G http://http.speedtest.us-east-01.coreweave.com/10G", stream=True)
...
def _(os, shell, time):
    def run_s5cmd_upload_test(numworkers: int = 16, concurrency: int = 32):
        ...
        print(f"--- s5cmd Upload Test ---")
        print(f"   Bucket: s3://{bucket}")
        print(f"   Workers: {numworkers} (--numworkers)")
        print(f"   Concurrency: {concurrency} (--concurrency per file)\n")
        
        start = time.time()
        shell(f"""s5cmd --endpoint-url {endpoint} --numworkers {numworkers} \
            cp --concurrency {concurrency} /tmp/bandwidth-test/10G 's3://{bucket}/benchmark/10G'""", stream=True)
        elapsed = time.time() - start
        ...
        shell(f"s5cmd --endpoint-url {endpoint} rm 's3://{bucket}/benchmark/10G'", quiet=True)
...
""")
        ...
        print(f"--- Rclone Upload Test ---")
        print(f"   Transfers: {transfers}")
        print(f"   Upload Concurrency: {upload_concurrency}")
        print(f"   Chunk Size: {chunk_size_mb}M\n")
        
        start = time.time()
        shell(f"""rclone copy /tmp/bandwidth-test/10G {remote}{bucket}/benchmark/ \
            --progress \
            --stats 5s \
            --transfers {transfers} \
            --s3-chunk-size {chunk_size_mb}M \
            --s3-upload-concurrency {upload_concurrency}""", stream=True)
        elapsed = time.time() - start
